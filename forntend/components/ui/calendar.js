...
function Calendar({ className, classNames, showOutsideDays= true, captionLayout= 'label', buttonVariant= 'ghost', formatters, components, ...props }) {
    ...
    return (_jsx(DayPicker, { showOutsideDays, className: cn('bg-background group/calendar p-3 [--cell-size:--spacing(8)] [[data-slot=card-content]_&]:bg-transparent [[data-slot=popover-content]_&]:bg-transparent', String.raw `rtl:**:[.rdp-button\_next>svg]:rotate-180`, String.raw `rtl:**:[.rdp-button\_previous>svg]:rotate-180`, className), captionLayout, formatters{
            formatMonthDropdown: (date)=> date.toLocaleString('default', { month),
            ...formatters,
        }, classNames{
            ...
            button_previous: cn(buttonVariants({ variant), 'size-(--cell-size) aria-disabled:opacity-50 p-0 select-none', defaultClassNames.button_previous),
            button_next: cn(buttonVariants({ variant), 'size-(--cell-size) aria-disabled:opacity-50 p-0 select-none', defaultClassNames.button_next),
            ...
            Root: ({ className, rootRef, ...props })=> {
                return (_jsx("div", { "data-slot", className: cn(className), ...props }));
            },
            ...
            WeekNumber: ({ children, ...props })=> {
                return (_jsx("td", { ...props, children: _jsx("div", { className: "flex size-(--cell-size) items-center justify-center text-center", children) }));
            },
            ...components,
        }, ...props }));
}
function CalendarDayButton({ className, day, modifiers, ...props }) {
    ...
    return (_jsx(Button, { ref, variant, size, "data-day": day.date.toLocaleDateString(), "data-selected-single": modifiers.selected &&
